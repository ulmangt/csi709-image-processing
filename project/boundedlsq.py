...
Directly Based on the code from:
     Ola HÃ¤rkegÃ¥rd, www.control.isy.liu.se/~ola   
    see licsence. 
...
"""
 ...
 #m = number of variables (actuators)
    ...
    A=np.dot(np.dot(Wv,B),np.linalg.pinv(Wu))    
    #print B, v
    #A=B    
    b = np.dot(Wv,(v-np.dot(B,ud)))
    #b=v
    #print b
    xmin = (umin-ud)
    xmax = (umax-ud)
    ...
    for i in range(imax):  
        ...
        if phase==1:
            ...
            if m_free<=k:                
                ...
                q1,r1=qr(A_free.T)
                p_free=-np.dot(q1,(np.solve(r1.T,r)))
            p=np.zeros((m,1))
            ...
            if phase==1 and m_free>=k:
                ...
                if m_free<m:
                    if phase==1:
                        ...
                        lam[i_fixed]=-W[i_fixed]*np.solve(R,np.dot(V1.T,z))
